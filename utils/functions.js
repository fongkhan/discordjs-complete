...
function rollAllDice(message, dices, explode, keep, adding) {
	...
	const nbdiceroll = dices.split('+').split('-').split('*').split('/');
	let result = 0;
	// for each dices to roll
	for (let dice = 0; dice < nbdiceroll; dice++) {
		// get the number of dices and the number of faces for the roll
		const nbdice = dice.split('d')[0];
		const maxface = dice.split('d')[1];
		// roll dices
	...
	console.log(result);
	message.reply({ content: `Result of ${interaction.member.name} is : ${result}`, ephemeral: true });
}